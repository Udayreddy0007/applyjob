  ...
  return (
    <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px", flexWrap: "wrap", gap: "12px" }}>
        ...
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          <button className="btn btn-primary" style={{ fontSize: "12px", padding: "8px 16px" }} onClick={onFetchJobs} disabled={loadingJobs || !resume}>
            {loadingJobs ? <><span className="spinner" /> Finding...</> : "⬡ Find Jobs from Resume"}
          </button>
      ...
      <div className="card" style={{ marginBottom: "16px", background: "#0a1628", border: "1px solid #1e3f5a" }}>
        <div style={{ fontSize: "12px", color: "#7dd3fc", marginBottom: "10px", fontWeight: "600" }}>🔍 Search for any job role</div>
        <form onSubmit={handleCustomSearch} style={{ display: "flex", gap: "10px" }}>
          <input
            type="text"
            placeholder="e.g. React Developer, Data Analyst, Python Intern, Marketing..."
            value={customSearch}
          ...
          />
          <button type="submit" className="btn btn-primary" style={{ padding: "10px 20px", fontSize: "13px" }} disabled={loadingJobs || !customSearch.trim()}>
            {loadingJobs ? <><span className="spinner" /> Searching...</> : "Search Jobs →"}
          </button>
        </form>
        <div style={{ marginTop: "8px", display: "flex", gap: "6px", flexWrap: "wrap" }}>
          {["React Developer", "Python Intern", "Data Analyst", "UI/UX Designer", "Node.js", "Machine Learning", "Digital Marketing", "Java Developer"].map((s) => (
      ...
      <div style={{ display: "flex", gap: "10px", marginBottom: "12px", flexWrap: "wrap" }}>
        <input
          type="text"
          placeholder="Filter loaded jobs by title, company or skill..."
          value={search}
        ...
        />
        <button onClick={() => setFresherOnly(!fresherOnly)}
          style={{ padding: "10px 16px", border: `1px solid ${fresherOnly ? "#10b981" : "#1e293b"}`, borderRadius: "8px", cursor: "pointer", fontFamily: "'DM Mono', monospace", fontSize: "12px", background: fresherOnly ? "#10b981" : "#0f1623", color: fresherOnly ? "#080c14" : "#64748b", transition: "all 0.2s" }}>
          🎓 Fresher {fresherCount > 0 ? `(${fresherCount})` : ""}
        </button>
      </div>
      ...
      {!resume && (
        <div style={{ background: "#0d1f35", border: "1px solid #1e3f5a", borderRadius: "10px", padding: "14px 18px", marginBottom: "20px", fontSize: "12px", color: "#7dd3fc" }}>
          ⬡ Upload your resume in Dashboard to find personalized jobs, or use the search box above to search any role.
        </div>
      )}
      ...
      {jobs.length === 0 && (
        <div className="card" style={{ textAlign: "center", padding: "50px" }}>
          ...
          <div style={{ fontSize: "12px", color: "#475569", marginBottom: "16px" }}>Search for a job role above, or go to Dashboard → Find Matching Jobs.</div>
        </div>
      )}
      ...
      {filtered.length === 0 && jobs.length > 0 && (
        <div className="card" style={{ textAlign: "center", padding: "40px" }}>
          ...
          <div style={{ fontSize: "13px", color: "#475569" }}>No jobs match your filter. Try clearing the search.</div>
        </div>
      )}
          ...
          <div key={job.id} className={`job-card${selectedId === job.id ? " selected" : ""}`} onClick={() => setSelectedId(selectedId === job.id ? null : job.id)}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "12px" }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px", flexWrap: "wrap" }}>
                  ...
                  {job.fresherFriendly && <span style={{ background: "#064e3b", color: "#4ade80", border: "1px solid #166534", padding: "2px 8px", borderRadius: "20px", fontSize: "10px", fontWeight: "600" }}>🎓 Fresher</span>}
                ...
                <div style={{ fontSize: "12px", color: "#64748b", marginBottom: "8px" }}>{job.company} · {job.location}{job.salary && job.salary !== "Not specified" ? ` · ${job.salary}` : ""}</div>
              ...
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                {job.match != null ? (
                  <div>
                    <div className="score-badge" style={{ background: matchColor(job.match) + "22", color: matchColor(job.match), border: `1px solid ${matchColor(job.match)}44`, display: "inline-block", marginBottom: "6px" }}>{job.match}% match</div>
                    <div className="progress-bar" style={{ width: "80px" }}><div className="progress-fill" style={{ width: `${job.match}%`, background: matchColor(job.match) }} /></div>
                  </div>
            ...
            {selectedId === job.id && (
              <div style={{ marginTop: "16px", paddingTop: "16px", borderTop: "1px solid #1e293b" }}>
                <div style={{ fontSize: "12px", color: "#94a3b8", whiteSpace: "pre-wrap", lineHeight: "1.8", marginBottom: "16px", maxHeight: "200px", overflowY: "auto" }}>
                  {job.description || "No description available."}
                </div>
                {job.matchReason && <div style={{ fontSize: "11px", color: "#7dd3fc", background: "#0d1f35", border: "1px solid #1e3f5a", borderRadius: "6px", padding: "8px 12px", marginBottom: "8px" }}>⬡ {job.matchReason}</div>}
                {job.matchingSkills?.length > 0 && <div style={{ fontSize: "11px", color: "#4ade80", background: "#061a0e", border: "1px solid #166534", borderRadius: "6px", padding: "8px 12px", marginBottom: "8px" }}>✓ Matching: {job.matchingSkills.join(", ")}</div>}
                {job.missingSkills?.length > 0 && <div style={{ fontSize: "11px", color: "#f87171", background: "#1f0a0a", border: "1px solid #3b1212", borderRadius: "6px", padding: "8px 12px", marginBottom: "14px" }}>⚠ Missing: {job.missingSkills.join(", ")}</div>}
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                  {!job.applied ? (
                    <>
                      ...
                      {job.url && <a href={job.url} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}><button className="btn btn-ghost">↗ View Listing</button></a>}
                    </>
